 ...
 *
 * Modelos não são mais validados contra catálogo estático —
 * a validação acontece em runtime contra a API OpenRouter.
 */
...
OPCOES:
  ...
  -c, --context <caminhos> Arquivos/dirs de contexto, separados por virgula
      ...
      --worker <modelo>    Modelo para os workers (ex: anthropic/claude-haiku-4-5)

PRECEDENCIA:
  Flags CLI > config persistida (~/.pi-dag-cli.json) > defaults

MODELOS:
  Todos os modelos disponiveis na OpenRouter sao listados em tempo real
  durante a configuracao. Use o ID no formato "provider/model-name".

EXEMPLOS:
  pi-dag
  pi-dag --task "Adicionar autenticacao JWT"
  pi-dag -t "Refatorar modulo de pagamento" -c src/payments,src/utils
  pi-dag --planner google/gemini-3.1-pro --worker anthropic/claude-haiku-4-5
`.trimStart();
