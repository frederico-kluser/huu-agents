 ...
 * Retorna erro se perfil não existir no catálogo do scope indicado.
 *
 * @param profileId - ID do perfil a remover
 * @param scope - Escopo do catálogo (global ou project)
 * @param rootPath - Raiz do projeto (obrigatório se scope = project)
 * @returns Result vazio em sucesso ou erro tipado
 *
 * @example
 * const result = await deleteProfile('test-driven-fixer', 'project', '/home/user/my-project');
 * if (result.ok) console.log('Removido');
 */
