...
/** Options for customizing model fetching behavior */
export interface FetchModelsOptions {
  /** Minimum Unix timestamp for model creation date filter. Default: 2025-01-01 */
  readonly minCreatedTimestamp?: number;
  /** Whether to exclude free model variants (`:free` suffix). Default: true */
  readonly excludeFreeVariants?: boolean;
  /** Whether to require non-zero pricing. Default: true */
  readonly requirePricing?: boolean;
}
...
const DEFAULT_MIN_CREATED = 1735689600;
...
/**
 * Busca modelos de texto da OpenRouter.
 ...
 * @param options - Options for filtering models
...
export const fetchOpenRouterModels = async (
  ...
  const minCreated = options?.minCreatedTimestamp ?? DEFAULT_MIN_CREATED;
  const excludeFree = options?.excludeFreeVariants ?? true;
  const requirePrice = options?.requirePricing ?? true;
  ...
  try {
    ...
    try {
      ...
      const filtered = parsed.data.data.filter((m) => {
        ...
        const isRecent = m.created >= minCreated;
        const isFreeVariant = excludeFree && m.id.endsWith(':free');
        const hasPrice = !requirePrice || parseFloat(m.pricing.prompt) > 0 || parseFloat(m.pricing.completion) > 0;
        return isTextOutput && isRecent && !isFreeVariant && hasPrice;
      });
